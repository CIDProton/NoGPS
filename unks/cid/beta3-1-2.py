...
class tracker:
    ...
    def run(tracklog):
        ...
        KP = 0.85          # Пропорциональный: дает сильный пинок при отдалении
        KI = 0.020         # Интегральный: дотягивает миллиметры, если радар не дошел
        KD = 0.80          # Дифференциальный: ГАСИТ инерцию, жестко тормозит перед нулем
        I_MAX = 4000.0     # Лимит накопления ошибки

        MIN_SPD = 1
        MAX_SPD = 100      # Максимальная скорость
        SEARCH_SPD = 40    # Скорость поиска (быстрая, но не 100, чтобы не проскочить маркер)
        
        ALPHA = 0.85      # Меньше сглаживания (было 0.35) -> быстрее реакция, нет отставания
