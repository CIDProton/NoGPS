...
private:
    ...
    float newNodeDist = 120.0f;           // расстояние до нового узла
    float reflexDist = 50.0f;             // дистанция рефлекса
    float mergeTolerance = 10.0f;          // порог слияния глобальных линий
...
// Конфигурация
constexpr int kLeftPanelWidth = 640;          // фиксированная ширина левой панели
constexpr float kFrameDt = 0.016f;             // ~60 FPS
float kCoreViewScale = 0.6f;                   // масштаб правой панели (можно менять)

// Размеры карты (левая панель)
const int MAP_W = 640;
const int MAP_H = 800;
std::vector<bool> worldMap(MAP_W * MAP_H, false);
uint32_t mapPixels[MAP_H][MAP_W];               // пиксельный буфер
...
Vec2 autoTargetDir(1.0f, 0.0f);
int autoChangeTimer = 0;

// Отображение глобальных линий
...
// ---------------------------------------------------------------------
// Рисовальные примитивы (без изменений)
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// DDA Ray marching (с увеличенной дальностью)
// ---------------------------------------------------------------------
float CastRay(Vec2 start, Vec2 dir) {
    ...
    const float maxDist = 500.0f; // увеличено
...
// ---------------------------------------------------------------------
// Преобразование в координаты правой панели (адаптивное к размеру окна)
// ---------------------------------------------------------------------
POINT toCoreView(Vec2 worldPoint, Vec2 center, int winW, int winH) {
    int rightPanelWidth = winW - kLeftPanelWidth;
    int rightPanelHeight = winH;
    float panelCenterX = kLeftPanelWidth + rightPanelWidth / 2.0f;
    float panelCenterY = rightPanelHeight / 2.0f;

    float localX = (worldPoint.x - center.x) * kCoreViewScale;
    float localY = (worldPoint.y - center.y) * kCoreViewScale;

    POINT pt;
    pt.x = static_cast<LONG>(panelCenterX + localX);
    pt.y = static_cast<LONG>(panelCenterY + localY);
    return pt;
}

// ---------------------------------------------------------------------
// Оконная процедура (обрабатываем WM_SIZE)
// ---------------------------------------------------------------------
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        ...
        case WM_SIZE: {
            ...
            resized = true;
            // Сообщим приложению, что нужно перерисовать
...
// Отрисовка кадра
// ---------------------------------------------------------------------
void drawFrame(HDC hdcMem, HDC hdcWindow, const std::vector<LidarPoint>& scan, int winW, int winH) {
    ...
    // ---- Левая панель: карта (пиксельный буфер) ----
    BITMAPINFO bmi = {};
    ...
    // Глобальные линии (если включены)
    if (showGlobalLines) {
        const auto& globalLines = core.getGlobalLines();
        for (const auto& line : globalLines) {
            POINT p1 = toCoreView(line.start, estPos, winW, winH);
            POINT p2 = toCoreView(line.end, estPos, winW, winH);
            DrawLine(hdcMem, p1.x, p1.y, p2.x, p2.y, RGB(255, 255, 0), 1);
        }
    }

    // Текущие линии признаков (голубые)
    ...
    RECT textRect = {kLeftPanelWidth + 10, winH - 200, winW - 10, winH - 10};
    DrawText(hdcMem, logs.c_str(), -1, &textRect, DT_LEFT);

    // Пояснительные надписи
    TextOut(hdcMem, 10, 10, "SIMULATION REALITY", 18);
    TextOut(hdcMem, kLeftPanelWidth + 10, 10, "CORE MEMORY VIEW", 16);
    TextOut(hdcMem, kLeftPanelWidth + 10, 30, "Centered on estimated drone position", 35);
    if (autoExplore) {
        TextOut(hdcMem, kLeftPanelWidth + 10, 50, "AUTO EXPLORE MODE ON", 20);
    }
    if (showGlobalLines) {
        TextOut(hdcMem, kLeftPanelWidth + 10, 70, "Global lines: ON", 16);
    } else {
        TextOut(hdcMem, kLeftPanelWidth + 10, 70, "Global lines: OFF", 16);
    }

    // FPS в заголовок окна
    char title[256];
    sprintf(title, "NoGPS Sim | FPS: %.1f | Nodes: %zu | Global: %zu",
            currentFps, graph.size(), core.getGlobalLines().size());
    SetWindowText(GetParent(hdcWindow), title);

    // Перенос буфера
    BitBlt(hdcWindow, 0, 0, winW, winH, hdcMem, 0, 0, SRCCOPY);
}
    ...
    for (int y = 1; y < MAP_H - 1; y++) {
        for (int x = 1; x < MAP_W - 1; x++) {
            if (!worldMap[y * MAP_W + x]) {
                ...
                core.reset(); // сбрасываем ядро
                core = DroneCore(); // но reset уже должен обнулить, для надёжности
                return;
            }
        }
    }
...
int runSimulator() {
    ...
    HBITMAP hbmMem = CreateCompatibleBitmap(hdcWindow, windowWidth, windowHeight);
    ...
    while (running) {
        ...
        if (resized) {
            ...
            DeleteObject(hbmMem);
            hbmMem = CreateCompatibleBitmap(hdcWindow, windowWidth, windowHeight);
        ...
        // Включение/выключение автоисследования (клавиша E)
        ...
        if (GetAsyncKeyState('R') & 0x8000) {
            if (!rPressed) {
                initWorld(); // заново генерируем карту и сбрасываем ядро
                rPressed = true;
            }
        } else {
            rPressed = false;
        ...
        // Показать/скрыть глобальные линии (клавиша T)
        static bool tPressed = false;
        if (GetAsyncKeyState('T') & 0x8000) {
            if (!tPressed) {
                showGlobalLines = !showGlobalLines;
                tPressed = true;
            }
        } else {
            tPressed = false;
        ...
        // Автономный режим
        if (autoExplore) {
            // Если таймер истёк или дрон близко к стене, меняем направление
            autoChangeTimer--;
            if (autoChangeTimer <= 0 || CastRay(dronePos, autoTargetDir) < 30.0f) {
                // Ищем случайное безопасное направление
                for (int tries = 0; tries < 20; tries++) {
                    float angle = (rand() % 360) * PI / 180.0f;
                    Vec2 dir(cos(angle), sin(angle));
                    if (CastRay(dronePos, dir) > 50.0f) {
                        autoTargetDir = dir;
                        autoChangeTimer = 60 + rand() % 60; // 1-2 секунды при 60 FPS
                        break;
                    }
                }
            }
            inputVel = autoTargetDir * 80.0f; // скорость в авторежиме
        }
        ...
        for (int i = 0; i < kNumRays; i++) {
            ...
            float noise = (autoExplore ? 0.0f : ((std::rand() % 100) / 100.0f) * 3.0f); // без шума в авторежиме
        ...
        // Отрисовка
        drawFrame(hdcMem, hdcWindow, scan, windowWidth, windowHeight);
        ...
        Sleep(4); // ~250 FPS макс, но управляется монитором
    }
